...
- **What tradeoff does it introduce?** **Safety vs. Speed.** Strict `fsync()` guarantees durability but introduces a <!-- DYNAMIC:SYNC_TAX -->**454x**<!-- END_DYNAMIC --> performance tax compared to OS buffering.
...
**Result:** Enabling `fsync()` introduces a <!-- DYNAMIC:SYNC_TAX -->**454x**<!-- END_DYNAMIC --> reduction in throughput.
...
**Result:** Adopting faster recovery logic (`kTolerateCorruptedTailRecords`) yields a <!-- DYNAMIC:RECOVERY_REDUCTION -->**2.1x**<!-- END_DYNAMIC --> reduction in Mean Time To Recovery (MTTR) baseline overhead (measured via clean-shutdown).
